...
void higher(int *subArray, int *previousArray, int size) {
  ...
  if (size > 0) {
    ...
    for (i = size; i <= fusionSize; i++)
      subArray[i - size] = fusionArray[i];
...
int main(int argc, char *argv[]) {
  ...
  if (rank == 0) {
    ...
    MPI_Scatter(arrayToSort, N / hostCount, MPI_INT, subArray, N / hostCount,
  ...
  MPI_Scatterv(arrayToSort, sendcnts, displs, MPI_INT, subArray, N / hostCount,
               MPI_INT, 0, MPI_COMM_WORLD);
  free(displs);
  free(sendcnts);
  ...
  MPI_Gather(subArray, N / hostCount, MPI_INT, arrayToSort, N / hostCount,
             MPI_INT, 0, MPI_COMM_WORLD);
  if (rank == 0) {
