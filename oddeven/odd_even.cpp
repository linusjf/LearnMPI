...
void compare_split_min(int cmp_proc_id, container &sub_array) {
  ...
  // std::cout << "#" << proc_id << " send array to " << cmp_proc_id <<
  // std::endl;
  MPI_Send(&sub_array[0], sub_array.size(), MPI_INT, cmp_proc_id, 0,
           MPI_COMM_WORLD);

  // std::cout << "#" << proc_id << " wait for array from " << cmp_proc_id <<
  // std::endl;

  MPI_Recv(&other_sub_array[0], other_sub_array.size(), MPI_INT, cmp_proc_id, 0,
  ...
  merged.insert(merged.end(), sub_array.begin(), sub_array.end());
  merged.insert(merged.end(), other_sub_array.begin(), other_sub_array.end());
...
int main(int argc, char **argv) {
  ...
  MPI_Scatter(&array[0], ELEMENTS_PER_PROCESS, MPI_INT, &sub_array[0],
              ELEMENTS_PER_PROCESS, MPI_INT, 0, MPI_COMM_WORLD);
  // std::cout << "#" << proc_id << ", sub_array: " <<
  // array_to_string(sub_array) << std::endl;

  // sort
  std::sort(sub_array.begin(), sub_array.end(),
            std::less<container::value_type>());
  // std::cout << "#" << proc_id << ", sorted sub_array: " <<
  // array_to_string(sub_array) << std::endl;
  ...
  for (int i = 0; i < world_size; i++) {
    ...
    if (i % 2 == 1) {
      ...
      if (proc_id % 2 == 1) {
        if (proc_id < world_size - 1) {
          // compare this process with right one.  save lesser part of the
          // joined array.
          compare_split_min(proc_id + 1, sub_array);
        }
      } else if (proc_id > 0) {
        // compare this process with left one. save greater part of the joined
        // array.
        compare_split_max(proc_id - 1, sub_array);
      }
      //  std::cout << "#" << proc_id << ", Odd exchange complete, i : " << i <<
      //  std::endl;
    }
    // even iteration (p0, p1), (p2, p3)
      ...
      if (proc_id % 2 == 0) {
        if (proc_id < world_size - 1) {
          // compare this process with right one. save lesser part of the joined
          // array.
          compare_split_min(proc_id + 1, sub_array);
        }
      } else if (proc_id > 0) {
      ...
      //  std::cout << "#" << proc_id << ", Even exchange complete, i : " << i
      //  << std::endl;
    }
  }

  MPI_Gather(&sub_array[0], ELEMENTS_PER_PROCESS, MPI_INT, &array[0],
             ELEMENTS_PER_PROCESS, MPI_INT, 0, MPI_COMM_WORLD);

  auto end_time = std::chrono::high_resolution_clock::now();

  if (proc_id == ROOT_RANK) {
    std::cout << "MPI sort: " << std::endl
              << array_to_string(array) << std::endl;
    std::cout << "Time for parallel sorting : "
    ...
    std::cout << "Non-MPI sort: " << std::endl
              << array_to_string(array_copy) << std::endl;
    std::cout << "Time for standard sorting : "
              << duration_cast<std::chrono::milliseconds>(end_time - start_time)
                     .count()
              << "ms" << std::endl;
  }
